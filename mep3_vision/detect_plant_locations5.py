...
from models.cnn_tracking8 import CNNTracking8
...
board_image_hor = cv2.rotate(board_image, cv2.ROTATE_180)
board_image_hor = cv2.resize(board_image_hor, (300, 200), interpolation=cv2.INTER_AREA)

board_image_vert = cv2.rotate(board_image, cv2.ROTATE_90_CLOCKWISE)
board_image_vert = cv2.resize(board_image_vert, (200, 300), interpolation=cv2.INTER_AREA)

model = CNNTracking8().to(device)
model.eval()
...
checkpoint = torch.load('model_weights/tracking_v8.pth', map_location=torch.device(device))
...
def get_plant_locations():
    ...
    occupancy_map = cv2.resize(occupancy_map, (board_image_hor.shape[1], board_image_hor.shape[0]), interpolation=cv2.INTER_NEAREST)
    plant_map = cv2.resize(plant_map, (board_image_hor.shape[1], board_image_hor.shape[0]), interpolation=cv2.INTER_NEAREST)
    location_display = cv2.resize(location_display, (board_image_vert.shape[1], board_image_vert.shape[0]), interpolation=cv2.INTER_NEAREST)
    circle_radius = int(unadjusted_plant_area_radius * board_image_vert.shape[0])

    for l in range(total_plant_areas):
        circle_x, circle_y = int(unadjusted_coords[l][0] * board_image_vert.shape[1]), int(unadjusted_coords[l][1] * board_image_vert.shape[0])
    ...
    board_occupancy_image = cv2.addWeighted(board_image_hor, 1.0, grayscale_3channel_occupancy, 0.8, 0.0)
